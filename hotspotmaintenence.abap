...
SELECTION-SCREEN BEGIN OF BLOCK b02 WITH FRAME TITLE TEXT-006.
...
CLASS lcl_int_maint DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    METHODS display_acdoca_lifnr IMPORTING is_int_upl TYPE ty_intent_upl.
    ...
    METHODS handle_db_click FOR EVENT double_click OF cl_gui_alv_grid
      IMPORTING e_row
                e_column .
...
CLASS lcl_int_maint IMPLEMENTATION.
  METHOD maintain.
    ...
    DATA :lv_dec2   TYPE p DECIMALS 2.
    ...
    LOOP AT mt_intent_uplt ASSIGNING FIELD-SYMBOL(<ls_int>).
      ...
      IF <ls_int>-z_plafond <> 0.
        lv_dec2 = ( <ls_int>-z_balance * 100 ) / <ls_int>-z_plafond.
      ENDIF.
      <ls_int>-percentage = |{ lv_dec2 } %|.
    ENDLOOP.
...
* Implement suitable error handling here
  ...
  METHOD handle_db_click.
    ...
    CASE e_column.
      WHEN 'Z_VV' OR 'NAME1'.
        READ TABLE mt_intent_uplt ASSIGNING FIELD-SYMBOL(<ls_int>) INDEX e_row-index.
        IF sy-subrc <> 0.
          RETURN.
        ENDIF.

        IF <ls_int>-z_vv IS INITIAL.
          RETURN.
        ENDIF.

        display_acdoca_lifnr( <ls_int> ).
    ENDCASE.

  ENDMETHOD.

  METHOD display_acdoca_lifnr.
    DATA: lr_belnr       TYPE RANGE OF acdoca-belnr,
...
* EXTRACTION 1
    DATA(lv_gjahr) = CONV acdoca-gjahr( is_int_upl-z_from(4) - 1 ).
    ...
    FROM acdoca
    INTO TABLE @lr_belnr
      WHERE lifnr  = @is_int_upl-z_vv
        AND rbukrs = @is_int_upl-z_cc
        AND gjahr  = @lv_gjahr
        AND belnr  IN @lr_tvarv_belnr
        AND bldat  BETWEEN @is_int_upl-z_from AND @is_int_upl-z_to.
...
*          EXTRACTION 2
    ...
    IF mt_acdoca IS INITIAL.
      MESSAGE 'No data found'(007) TYPE 'I'.
      RETURN.
    ENDIF.
