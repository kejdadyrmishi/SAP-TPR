...
* Get dynamic fields for header, from DB table
...
* Obligatory fields for items
    mt_itm_map = VALUE #(
      ( fieldname = cv_sales_ord tabname = cv_tab_so  )
      ( fieldname = cv_material  tabname = cv_tab_itm )
    ).

* Get dynamic fields for items, from DB table
...
*        Get partner number description based on when AG or WE
        WHEN cv_tab_partn.
...
* Items Components
  ...
  METHOD constructor.
    ...
    ls_button = VALUE #( icon_id   = icon_export
                         icon_text = 'Download Template'(005) ).
...
*            vbeln can not be empty
            MESSAGE 'Sales Document field can not be empty!'(006) TYPE 'S' DISPLAY LIKE 'E'.
            CONTINUE.
          ENDIF.
...
*          duplicate vbeln
          READ TABLE mt_bapi_so TRANSPORTING NO FIELDS WITH TABLE KEY vbeln = <lv_vbeln>.
          IF sy-subrc = 0.
            MESSAGE 'Duplicate Sales Document!'(007) TYPE 'S' DISPLAY LIKE 'E'.
            CONTINUE.
          ENDIF.
...
* loop second sheet = items => Fill bapi and output
        LOOP AT <lt_itm_excel> ASSIGNING FIELD-SYMBOL(<ls_itm_excel>) FROM 2.
          ...
          IF <lv_vbeln> IS INITIAL.
*            vbeln can not be empty
            MESSAGE 'Sales Document field can not be empty!'(008) TYPE 'S' DISPLAY LIKE 'E'.
            CONTINUE.
          ENDIF.
...
*          vbeln was not inserted in the header but only in items
          ...
          IF sy-subrc <> 0.
            MESSAGE 'Sales Document was not inserted in the header but only in items!'(009) TYPE 'S' DISPLAY LIKE 'E'.
            CONTINUE.
          ENDIF.
...
*            customizing template with errors - field does not exists in table
  ...
  METHOD adjust_cols_msg.
    ...
    TRY.
        DATA(lv_text) = CONV scrtext_l( 'Status'(004) ).
        ...
        lv_text = CONV scrtext_l( 'Excel Sales Document'(010) ).
